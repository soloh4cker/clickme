(() => {
  ...
  function listsAreEqual(first, second) {
  ...
  async function setOnlineCache(serverData) {
    ...
    const current = await chrome.storage.local.get(CACHE_KEY);
    const updates = { [STATUS_KEY]: status };
    if (!listsAreEqual(Array.isArray(current[CACHE_KEY]) ? current[CACHE_KEY] : [], guests)) {
      updates[CACHE_KEY] = guests;
    }
      ...
      try {
        ...
        if (allowMigration && !local[MIGRATION_KEY]) {
          if (serverGuests.length === 0 && oldLocalGuests.length > 0) {
            ...
            await chrome.storage.local.set({ [PRE_MIGRATION_BACKUP_KEY]: oldLocalGuests });
          }
  ...
  async function mutateSharedList(message) {
    try {
      switch (message.type) {
        ...
        case 'shared-list-replace':
          await serviceRequest('/replace', {
            method: 'POST',
            body: JSON.stringify({
              guests: Array.isArray(message.guests) ? message.guests.map(cleanGuest) : []
            })
          });
          break;
      ...
      const freshServerData = await serviceRequest('/guests', { method: 'GET' });
      return await setOnlineCache(freshServerData);
    } catch (error) {
  ...
  chrome.runtime.onInstalled.addListener(() => refreshSharedList().catch(() => {}));
  chrome.runtime.onStartup.addListener(() => refreshSharedList().catch(() => {}));
})();
